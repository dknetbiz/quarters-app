  ...
  return (
    <div className="pb-6">
      ...
      <div className="px-4 -mt-6 space-y-4">
        ...
        {/* Top row: compact stat cards (left) + type-wise status (right) */}
        <div className="grid grid-cols-2 gap-3 items-start">

          {/* Compact 2x2 stat cards */}
          <div className="grid grid-cols-2 gap-2">
            <StatCard
              icon={Building2} label="Total" value={stats.total}
            ...
            <StatCard
              ...
              accent="bg-rose-500" iconBg="bg-rose-50" iconColor="text-rose-600"
              onClick={() => navigate('/quarters?status=Vacant')}
            />
            <StatCard
              icon={Wrench} label="Repair" value={stats.repair}
          ...
          {typeEntries.length > 0 && (
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-3">
              <p className="text-[11px] font-semibold text-slate-500 uppercase tracking-wide mb-2.5">By Type</p>
                  ...
                  return (
                    <div key={type}>
                      <div className="flex items-center justify-between mb-0.5">
                        <span className="text-[11px] font-medium text-slate-700 truncate max-w-[55%]">{type}</span>
                        <span className="text-[10px] text-slate-400 flex-shrink-0">
                          <span className="text-emerald-600 font-semibold">{occ}</span>
            ...
            <div className="grid grid-cols-3 gap-2">
              {Object.entries(stats.byLocation).map(([loc, count]) => (
                <div key={loc} className="bg-slate-50 border border-slate-100 rounded-xl px-3 py-2.5">
                  <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wide">{loc || 'Unknown'}</p>
            ...
            <div className="flex gap-3 items-start">
              ...
              <div className="flex-1 min-w-0">
                ...
                <button
                  ...
                  className="mt-1.5 text-xs font-semibold text-amber-700 flex items-center gap-1"
                >
